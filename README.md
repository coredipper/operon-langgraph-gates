...
The three **Binding** clauses above are also captured as a machine-checked [Allium](https://github.com/juxt/allium) spec at [`specs/certificate-binding.allium`](./specs/certificate-binding.allium) (replay-equivalence as `invariant`s, A2A round-trip as a `contract` with `@invariant Roundtrip`). The behavioural obligations are covered by the existing suites; the spec's *exact parameter-shape* obligations — which a passing round-trip test would not catch — are enforced by [`tests/test_certificate_binding_contract.py`](./tests/test_certificate_binding_contract.py), propagated from the spec.
